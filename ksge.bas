' KSGE K.I.S.S. BIT STRIP GAME ENGINE VERSION 5.5 20190922
...
' COMPILE WITH FREEBASIC COMPILER (FBC) TESTED WITH VERSION 1.0.7 ON LINUX (UBUNTU 18.04) AND WINDOWS 10
...
' VIDEOCLIPS SHOULD BE ENCRYPTED WITH CCRYPT HERE IS AN EXAMPLE: ./ccrypt -e -K Str1pgame$areWonferful! YOURFOLDER/*.mkv ; take a look also to ksge-utils for some automation
...
'********************************************************
const C1 as string = "MARYLIN" 'model name wich should be equal to folder name
K1 = "eNtErYoUrKEYanditshouldbecomplex" 'key used for encrypt media content and activation file
Kh = "yourtemporarykey" ' key used for temporary activation file (helpme)
...
raddress(1) = "yourBTCaddress" 'address to check transaction for (where monetize)
raddress(2) = "anotherBTCaddress optional" 'address to check transaction for (where monetize)
totaladdr = 2 'if you entered 2 address put 2.... ecc...
...
const C5bis as string = "3" 'number of standard rows (in case Commnand(1) = 0
const C6 as integer = 1 'number of stages allowed for demo
'********************************************************
...
#ELSE
   CONST OS = "linux"
#ENDIF
print "KBSGE VERSION 5.5 FOR: " ; OS
sleep 1000
...
'ksbj is your game binary, in this case blackjack
#IFDEF __FB_WIN32__
...
print "KBSGE version 5.5 20190922 THIS VERSION WORKS ONLY WITH ENCRYPTED MEDIA CONTENT"
...
Function rnd_range (first As Double, last As Double) As Double 'random number inrange for play random clip
...
function play(fileName as string, pInstance as libvlc_instance_t ptr, pPlayer as libvlc_media_player_t ptr) as integer
   var pMedia = libvlc_media_new_path (pInstance, fileName) 
...
var pInstance = libvlc_new(0, NULL) 
var pPlayer = libvlc_media_player_new(pInstance)
